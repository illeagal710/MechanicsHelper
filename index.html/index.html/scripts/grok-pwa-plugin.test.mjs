...
// A workspace with no baked src/lib/og/site.json and no public/og.jpg, so tests
// that assert injector fallbacks (document title, host slug, placeholder card)
// exercise those paths instead of picking up this app's own OG identity.
const NEUTRAL_ROOT = mkdtempSync(join(tmpdir(), "grok-og-neutral-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: NEUTRAL_ROOT, site: {} });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: NEUTRAL_ROOT, site: {} });
  const chunks = [
